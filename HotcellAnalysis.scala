...
object HotcellAnalysis {
  ...
  def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
  {
    ...
    val stdDev = scala.math.sqrt((hotnessOfCellsDf.withColumn("sqre_cell", pow(col("cell_hotness"), 2)).select("sqre_cell").agg(sum("sqre_cell")).first().getDouble(0) / numCells) - scala.math.pow(avg, 2))
    ...
    // function to calculate G (Getis-Ord)
    var gScoreFunc = udf(
    ...
    // calculate G Score for each cell and sort desc
